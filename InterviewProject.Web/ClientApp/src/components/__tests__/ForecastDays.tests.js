...
describe('Location Search', () => {
    ...
    it('should display location and forecast', () => {
        ...
        const { getByTestId } = render(<ForecastDays locationName={locationName} forecastDays={forecastDays} isLoading={false} hasError={false} />);
        expect(getByTestId('forecastdays-list')).toBeInTheDocument();
    });

    it('should render error message', () => {
        const { getByTestId } = render(<ForecastDays locationName={'test'} forecastDays={[]} isLoading={false} hasError={true} />);
